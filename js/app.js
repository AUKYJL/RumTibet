    ...
    (() => {
        ...
        class SelectConstructor {
            ...
            selectCallback(selectItem, originalSelect) {
                selectItem.querySelector(".select__value").removeAttribute("data-pseudo-label");
...
***************************************************************************** */
        ...
        function menuLinkClick(e) {
            menuLinks.forEach((item => item.classList.remove("_active")));
            e.target.classList.toggle("_active");
            e.preventDefault();
        }
        function createHeaderBG() {
            if (window.scrollY >= 100) header.classList.add("_green-header"); else header.classList.remove("_green-header");
        }
        function removeSwiperSwiperStyle(className, width) {
            const items = document.querySelectorAll(`.${className}`);
            if (items[0].closest(".swiper-wrapper")) items.forEach((item => {
                item.classList.add("swiper-slide");
            })); else if (window.innerWidth > width) items.forEach((item => {
        ...
        window.addEventListener("scroll", (() => {
            createHeaderBG();
            removeSwiperSwiperStyle("item-body-blog", 1250);
            removeSwiperSwiperStyle("item-photo-report", 800);
        }));
        window.addEventListener("resize", (() => {
            if (window.innerWidth <= 980) menuList.setAttribute("data-spollers", ""); else {
                menuList.removeAttribute("data-spollers");
                menuList.querySelectorAll(".menu__item").forEach((e => e.querySelector("div").setAttribute("hidden", "")));
            }
        }));
